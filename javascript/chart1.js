var cityData; // Global variable for city data
var stateData; // Global variable for state data
...
// SVG Container
var svg = d3.select("#chart-container") // Select the chart-container div
    .append("svg")
...
d3.json("australia.json").then(function (json) {
    ...
    svg.selectAll("path")
        ...
        .on("mouseover", function (d) {
            d3.select(this).classed("highlighted", true); // When mouseover, add CSS class "highlighted"
            d3.selectAll("path").classed("faded", true); // Fade out all other states
            d3.select(this).classed("faded", false); // But current state shouldn't be faded
            ...
            stateTooltip.html("<strong>" + d.properties.name + "</strong><br>" +
                ...
                .style("transform", "translate(-50%, -50%)"); // Center the tooltip relative to the cursor
        })
        ...
        .on("mouseout", function () {
            d3.select(this).classed("highlighted", false); // When mouseout, remove CSS class "highlighted"
            d3.selectAll("path").classed("faded", false); // Restore all states to normal state
    ...
    var stateLabels = svg.selectAll(".state-label")
        ...
        .attr("y", function (d) {
            var centroid = path.centroid(d);
            var yOffset = 0; // Adjust the offset and optional position of the state
            ...
            if (d.properties.name === "Northern Territory") {
                ...
                yOffset = -15;
                // If the y coordinate of the center is greater than height/2, adjust the y offset downwards
            } else {
                yOffset = 14; // Otherwise, adjust y offset downwards
            }
            return centroid[1] + yOffset; // Returns the y coordinate of the center plus offset
        })
...
d3.csv("city_data.csv", function (d) {
    ...
    d3.csv("aus_city.csv", function (d) {
        ...
        svg.selectAll(".city-circle")
            ...
            .attr("r", 4)
            .on("click", function (d, i) { // Change to click event
...
function drawCityPieChart(cityName) {
    ...
    var years = Object.keys(selectedCityData[0]).filter(function (key) {
        return key !== "city" && key !== "total"; // Remove 'total' column
    });
    ...
    var chartSvg = d3.select("#chart-container") // Select the chart-container div
        .append("svg")
